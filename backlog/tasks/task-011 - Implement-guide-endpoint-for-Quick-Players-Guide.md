---
id: TASK-011
title: Implement /guide endpoint for Quick Players Guide
status: In Progress
assignee: []
created_date: '2026-04-28 23:36'
updated_date: '2026-04-28 23:36'
labels: []
dependencies: []
priority: medium
---

## Description

<!-- SECTION:DESCRIPTION:BEGIN -->
Create a "Read the Rules" endpoint at /guide that provides a quick players guide. This will likely be an alias or a dedicated page for the quick reference content.
<!-- SECTION:DESCRIPTION:END -->

## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 The endpoint https://phalanxduel.com/guide is accessible.
- [ ] #2 The page provides a quick player's guide, ideally based on the quick-reference content.
- [ ] #3 The page follows the site's styling and layout.
<!-- AC:END -->

## Definition of Done
<!-- DOD:BEGIN -->
- [ ] #1 The a11y is run agains the site and all claims are verified against the official rules and guidlines for Phalanx Duel
<!-- DOD:END -->
